...
export const PublicationsSection = ({ username }: PublicationsSectionProps) => {
  ...
  useEffect(() => {
    if (getTabKeyByIdx(selectedTab) === "questions") {
      USER.getPublications(username, "question").then(
    ...
    if (getTabKeyByIdx(selectedTab) === "news") {
      USER.getPublications(username, "news").then(
        (res: AxiosListResponse<TPost>) => {
          setPosts(res.data.results)
        }
      )
    ...
    if (getTabKeyByIdx(selectedTab) === "articles") {
      USER.getPublications(username, "article").then(
        (res: AxiosListResponse<TPost>) => {
          setPosts(res.data.results)
        }
      )
